...
servArr.addEventListener("click", (e) => {
...
bkNw.addEventListener("click", () => {
    if(cart.length===0){
        ...
        let cartItemsText = `<table style="width: 100%; border-collapse: collapse; border: solid 1px black;">
                                <tr>
                                    <th style="border: solid 1px black; padding: 0.5rem; text-align: center;" >Sr.No.</th>
                                    <th style="border: solid 1px black; padding: 0.5rem; text-align: center;" >Service Name</th>
                                    <th style="border: solid 1px black; padding: 0.5rem; text-align: center;" >Price</th>
                                </tr>`;
        let j = 0;
        cart.forEach((item, index) => {
            j += item.servPrice;
            cartItemsText += `
                <tr>
                    <td style="border: solid 1px black; padding: 0.5rem; text-align: center;" >${index+1}</td>
                    <td style="border: solid 1px black; padding: 0.5rem; text-align: left;" ><b>${item.servName}</b></td>
                    <td style="border: solid 1px black; padding: 0.5rem; text-align: right;" ><i>₹ ${item.servPrice.toFixed(2)} /-</i></td>
                </tr>
            `;
        });
        cartItemsText += `
                        <tr>
                            <td colspan="2" style="text-align: center; border: solid 2px green; padding: 0.5rem;">Total Amount:</td>
                            <td style="text-align: right; padding: 0.5rem; border: solid 2px green;"><b><i><u>₹ ${j.toFixed(2)} /-</u></i></b></td>
                        </tr>
                        `;
        ...
        const emailTemplateParams = {
            ...
            cart_summary: cartItemsText
        };

        emailjs.send("service_pm0n67h", "template_lkdtzpp", emailTemplateParams)
            .then((response) => {
            ...
            .catch((error) => {
                ...
                des.classList.add("red");
                des.innerHTML = `<p>⚠️ Booking failed to dispatch. Please check your connection.</p>`;
            });
    }
});

nameInput.addEventListener("input", bkNwChecker);
emailInput.addEventListener("input", bkNwChecker);
phNoInput.addEventListener("input", bkNwChecker);
...
document.querySelector(".navbar ul").addEventListener("click", (e) => {
    ...
    if (!hrefValue || hrefValue==="#" || !hrefValue.startsWith("#")) return;

    const targetSection = document.querySelector(hrefValue);
    
    if (targetSection) {
        targetSection.classList.add("glow-highlight");

        setTimeout(() => {
           targetSection.classList.remove("glow-highlight"); 
        }, 3000);
    }
});
...
subscribe.addEventListener("click", () => {
    if(subNameInput.checkValidity() && subEmailInput.checkValidity()){
        ...
        emailjs.send("service_pm0n67h", "template_u7cmkjn", emailTemplateParams)
            .then((response) => {
                console.log("SUCCESS!", response.status, response.text);
            ...
            .catch((error) => {
                ...
                info.innerHTML = `<p>⚠️ Subscription Failed. Please check your connection.</p>`;
            });
...
subNameInput.addEventListener("input", subscribeBtnChecker);
subEmailInput.addEventListener("input", subscribeBtnChecker);